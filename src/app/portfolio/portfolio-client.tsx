...
import type { ContenidoListado } from "@/utils/kontororu-adapter";
...
interface PortfolioItem {
  ...
  published: boolean;
  /*
   * El orden llegó a guardarse en tres columnas distintas según la época del
   * dashboard; se leen las tres y se cae a `created_at`. Opcionales porque
   * ninguna está garantizada en todas las filas.
   */
...
interface PortfolioClientProps {
  /** Últimos 3 artículos, resueltos en servidor. */
  recentArticles: ContenidoListado[];
  /** Últimos 3 casos de estudio, resueltos en servidor. */
  recentCases: ContenidoListado[];
}
...
export function PortfolioClient({ recentArticles, recentCases }: PortfolioClientProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const fetchedItems: PortfolioItem[] = itemsData || [];
        ...
        /*
         * Los artículos y casos ya no se piden aquí: llegan por props desde el
         * Server Component. La clave de Kontorōru es secreta y este archivo se
         * envía al navegador.
         */
      } catch (error) {
...
function BentoCoversShowcase({ items }: { items: ContenidoListado[] }) {
  const fallbackItems = [
